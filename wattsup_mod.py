...
class wattsup (object):
    ...
    def amqpCon(self):
        ...
        connection = pika.BlockingConnection(pika.ConnectionParameters(host = cfg.RabbitHost, port = cfg.Port, virtual_host = cfg.VHost, credentials = credentials))
            ...
            while True:
                ...
                if l[0:2] == '#d':
                    ...
                    m = [('time', now),
                         ...
                         ('frequency', float(vals[16])/10.0),
                         ('volt-amps', float(vals[17])/10.0)]
                    ...
                    if format == 'raw':
                        ...
                        n['type'] = 'pw'
                        #n['type_instance'] = ''
                        n['interval'] = float(interval)
                        n['dsnames'] = ['watts', 'volts', 'frequency', 'volt-amps']
                        n['values'] = [m[1][1], m[2][1], m[3][1], m[4][1]]
                        n = [n]
                        #n['values'] = [m[0][1][1]['watts'], m['volts'], m['frequency'], m['volt-amps']]
                        chan.basic_publish(exchange=cfg.Exchange, routing_key=cfg.RoutingKey, body=json.dumps(n))
